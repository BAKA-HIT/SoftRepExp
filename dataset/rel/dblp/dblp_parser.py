...
def parse_entity(dblp_path, save_path, type_name, features=None, save_to_csv=False, include_key=False):
    ...
    if save_to_csv:
        ...
        for record in results:
            ...
            cnt = sum([1 if len(x) > 0 else 0 for x in list(record.values())])
            weight_left = 0
            weight_right = 0
            if cnt == len(features):
               full_entity += 1
            ...
            weight = weight_left + random.random() * (weight_right - weight_left)
            row = ['::'.join(v) for v in list(record.values())] + [weight]
            writer.writerow(row)
        f.close()
...
def parse_article(dblp_path, save_path, save_to_csv=False, include_key=False):
    type_name = ['article']
    features = ['title', 'author', 'year', 'journal', 'pages', 'ee', 'url']
...
def parse_inproceedings(dblp_path, save_path, save_to_csv=False, include_key=False):
    ...
    features = ['title', 'author', 'year', 'booktitle', 'pages', 'ee', 'url']
    info = parse_entity(dblp_path, save_path, type_name, all_features, save_to_csv=save_to_csv, include_key=include_key)
...
def main():
    ...
    try:
        ...
        exit()
    parse_inproceedings(dblp_path, save_path, save_to_csv=True)
